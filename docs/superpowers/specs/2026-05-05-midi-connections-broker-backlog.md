...
- **`list_midi_devices` reading from MCB.** Today the tool stitches three sources MCP-side: direct `easymidi` OS port enumeration, a direct read of the shared mock-registry JSON, and an MCB UDS query for leases. The encapsulation wart is that the MCP duplicates MCB's data sources — both processes parse the registry and enumerate ports independently, so they can disagree about what's running. Fix: add `GET /v1/midi/ports` to MCB that aggregates OS ports + registry annotations + lease info into a single response; collapse `list_midi_devices` to a thin wrapper that calls the endpoint, layers on the MCP-local pool markers (which MCB doesn't know about), and returns. Stitching logic moves from `src/tools/list-devices.ts` into the shared MCB client.
