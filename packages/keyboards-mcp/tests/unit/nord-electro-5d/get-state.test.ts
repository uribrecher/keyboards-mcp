...
describe("Nord Electro 5D get_current_state", () => {
  it("returns 'not supported' message — Nord MIDI is one-way", async () => {
  ...
  it("returns the same not-supported message regardless of section filter", async () => {
    const device = nordModel.createDevice!();
    const result = await Promise.resolve(device.getState("organ"));
